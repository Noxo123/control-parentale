...
app.post('/api/settings', auth, (req, res) => {
  const daily = Number(req.body.daily_limit_minutes);
  ...
  log('settings', `Paramètres modifiés : ${daily} min, ${start}-${end}`);
...
app.post('/api/pin', auth, (req, res) => {
  ...
  db.prepare('UPDATE settings SET parent_pin=? WHERE id=1').run(pin);
...
app.post('/api/blocked-apps', auth, (req, res) => {
  ...
  try {
    const r = db.prepare('INSERT INTO blocked_apps(process_name) VALUES(?)').run(name);
    log('blocklist', `Application surveillée : ${name}.exe`);
    res.json(db.prepare('SELECT * FROM blocked_apps WHERE id=?').get(r.lastInsertRowid));
  } catch {
    res.status(409).json({ error: 'Déjà présente' });
  }
});

app.delete('/api/blocked-apps/:id', auth, (req, res) => {
  const item = db.prepare('SELECT * FROM blocked_apps WHERE id=?').get(Number(req.params.id));
  if (!item) return res.status(404).json({ error: 'Introuvable' });
  db.prepare('DELETE FROM blocked_apps WHERE id=?').run(item.id);
  log('blocklist', `Application retirée : ${item.process_name}.exe`);
  res.json({ ok: true });
});
...
// Endpoint public local uniquement pour l'agent. Aucun PIN n'est exposé.
app.get('/api/agent-config', (req, res) => {
  ...
  res.json({
    ...
    end_time: s.end_time,
    blocked_apps: db.prepare('SELECT process_name FROM blocked_apps WHERE enabled=1').all().map(x => x.process_name)
  });
});
...
app.post('/api/agent-event', (req, res) => {
  ...
  log(String(req.body.type || 'agent'), message);
  res.json({ ok: true });
});

// Express 5 : utiliser un middleware fallback plutôt que app.get('*'),
// qui provoque PathError avec les versions récentes de path-to-regexp.
